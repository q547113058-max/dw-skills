...
1. 不需要实现的需求不写代码，只说明跳过理由。
2. 仓库已有 helper、组件、类型、配置或相邻模式时优先复用。
3. 标准库能解决时不用自写基础逻辑。
4. 平台原生能力能解决时不用额外 JS、服务或依赖。
5. 已安装依赖能解决时优先使用；不要为少量代码新增依赖。
6. 能用一行或小函数清晰表达时，不建立类、工厂或配置层。
7. 最后才写最小可工作的自定义实现。

精简实现的前提是理解真实需求和调用链。不要用“少写代码”作为跳过输入验证、错误处理、安全审查、测试或根因修复的理由。
...
- 优先写简单直接的代码，不做投机抽象。
- 只有真实重复或复杂度出现后才添加抽象。
- 不添加未请求的接口、工厂、适配层、未来扩展点或一次性配置。
- Bugfix 先搜索相关调用者和共享入口；能在根因处修复时，不在每个调用点重复补丁。
- 删除死代码、重复逻辑或无用配置优先于新增替代层。
...
- `quick` 窄 UI、文案、文档或低风险配置变更：针对性人工检查或现有窄测试可能足够。
- `standard` 共享逻辑或数据行为：添加或更新自动化测试。
- `high-risk` 认证、支付、持久化或外部 API：测试正常、失败和边界场景。

`standard`、`high-risk` 或存在失败/阻塞时，把测试结果记录到当天日志。
...
- `quick` 使用独立 diff 审查轮即可；不为形式启用额外角色。
- `standard`、`high-risk` 的有意义变更要分离生成和评估。
...
- 用户明确要求或仓库有明确自动交付策略时，代码变更验证后通过 GitHub 更新项目。
